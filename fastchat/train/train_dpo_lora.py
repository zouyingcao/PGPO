...
def train():
    ...
    # Load data
    import os
    cache_path = os.path.dirname(os.path.realpath(data_args.data_path))
    dataset = load_dataset("json", data_files=data_args.data_path, cache_dir=cache_path)
    ...
    #     trainer.save_model() # 这行代码会保存整个模型，包括原始模型的权重和LoRA层的权重。这意味着保存的模型是一个合并的模型，它包含了原始模型的参数和通过LoRA训练更新的参数。
    # else:
